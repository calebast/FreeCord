...
class VoiceUiContractTests(unittest.TestCase):
    ...
    def test_linux_desktop_audio_uses_an_automatic_native_pipewire_capture(self) -> None:
        ...
        self.assertIn("linuxScreenAudioSourceSerials", main)
        self.assertIn('spawn("pw-dump"', main)
        self.assertIn('props["object.serial"]', main)
        self.assertIn("existingSourceSerials", main)
        self.assertIn("waitForNewLinuxScreenAudioSource", main)
        self.assertIn("createdSerials.length > 1", main)
        self.assertIn("linuxScreenAudioSourceSerial", main)
        self.assertIn("await releaseLinuxScreenAudio()", main)
        ...
        self.assertIn('`--target=${screenAudioTarget}`', main)
        self.assertIn("did not produce PCM", main)
        self.assertIn('mainWindow.webContents.send("audio:linux-screen-data"', main)
        self.assertIn("MediaStreamTrackGenerator", voice)
        self.assertIn('new AudioData({ format: "s16"', voice)
        self.assertIn("numberOfChannels: 1", voice)
        self.assertIn("Math.round((left + right) / 2)", voice)
        ...
        self.assertIn("new LocalAudioTrack(generator", voice)
        self.assertIn('generator.contentHint = "music"', voice)
        self.assertIn("selected speaker is unavailable for stream audio", voice)
        local_share = voice[voice.index("const localVideo ="):voice.index("setScreenShareVolume")]
        self.assertNotIn("audioTrack:", local_share)
